...
import generateAbbreviations from "./BookAbbreviations.ts";
import P from "./Parsimmon.ts";

const makeBookLookup = (books: BookArray) => {
    const book_abbrevs = generateAbbreviations(books);
...
export const makeLanguage = (books: BookArray) => {
    const bl = makeBookLookup(books);
